...
- **The names on the card are Latin, and deliberately not from config.** `CARD_COUPLE_NAMES` in the build script is `Nicole & Dima`, set in serif small caps to match the invitation's own lettering. `wedding_config.couple_names` stays Hebrew because it titles the `.ics` event and the preview's bold text line. Blank the constant and the card falls back to config.
