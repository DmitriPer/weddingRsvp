...
- **⚠️ The guest data is real now, and there is only one database.** As of 2026-09-09 the Supabase project holds Dmitri's actual guest list — ~107 invitations, no `__test__` rows remaining — and there is no staging or test project to practise on. Treat every row as real: never bulk-write, seed or delete, never run `004_seed_test_data.sql`, and never fabricate or assume guest data. To verify a write path, make one reversible change to a single row and restore it immediately, or ask first; do not create throwaway rows. Irreversible operations need explicit confirmation every time, not once.

  *(This rule has been rewritten twice. It first said "mock data" — see the note below on the dropped mock store — then "no real guest data until told otherwise", which described a database holding only invented rows. Both earlier forms existed to keep real people's details safe; that purpose is unchanged, but the direction has flipped from keeping real data out to handling real data that is already in.)*
